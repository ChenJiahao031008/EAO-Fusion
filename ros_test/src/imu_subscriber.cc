...
IMUSubscriber::IMUSubscriber(ros::NodeHandle& nh, std::string topic_name, size_t buff_size)
    :nh_(nh) {
    subscriber_ = nh_.subscribe(topic_name, buff_size, &IMUSubscriber::imu_callback, this);
}

void IMUSubscriber::imu_callback(const sensor_msgs::ImuConstPtr& imu_msg_ptr) {
    buff_mutex_.lock();

    // convert ROS IMU to GeographicLib compatible GNSS message:
    sensor_msgs::Imu imu_data;
    imu_data.header.stamp = imu_msg_ptr->header.stamp;
...
void IMUSubscriber::ParseData(std::deque<sensor_msgs::Imu>& imu_data_buff) {
    buff_mutex_.lock();
